...
BEGIN
    ...
    IF v_students LIKE CONCAT('%"', v_studentIdStr, '"%') THEN
        ...
        SELECT CONCAT('{"success":true,"message":"退课成功","courseId":', p_courseId, ',"studentId":', p_studentId, '}') AS result;
    ELSE
        SELECT CONCAT('{"success":false,"error":"您未选择此课程","courseId":', p_courseId, ',"studentId":', p_studentId, '}') AS result;
    END IF;
END$$
