      ...
      <div v-if="activeTab === 'selected'" class="card">
        ...
        <div v-else>
          ...
          <div class="debug-info" style="background-color: #f0f0f0; padding: 10px; margin-bottom: 10px; font-size: 12px;">
            ...
            <p>已选课程数量: {{ processedSelectedCourses.length }}</p>
            <p>activeTab: {{ activeTab }}</p>
            <p>loading: {{ loading }}</p>
            <p>已选课程数据: {{ JSON.stringify(processedSelectedCourses) }}</p>
          </div>
          
          <!-- 已选课程列表 -->
          <div v-if="processedSelectedCourses.length > 0" class="selected-list">
            <h3 style="margin-bottom: 1rem; color: #333;">已选课程 ({{ processedSelectedCourses.length }}门)</h3>
            
            <div v-for="(course, index) in processedSelectedCourses" :key="index" class="selected-item">
          ...
          <!-- 学分总结 -->
          <div v-if="processedSelectedCourses.length > 0" class="credit-summary">
            <p>已选课程总数: {{ processedSelectedCourses.length }}</p>
            <p>总学分: {{ totalCredit }}</p>
          </div>
        </div>
      </div>
            ...
            <div class="course-grid">
              <div class="course-card" v-for="course in filteredCourses" :key="course.id">
                ...
                <button 
                  ...
                  :disabled="isCourseSelected(course.id) || getStudentCount(course) >= (course.maxStudents || 0) || selectingCourse === course.id"
                >
                  <span v-if="selectingCourse === course.id">
                    <i class="el-icon-loading"></i> 处理中...
                  </span>
                  <span v-else>
                    {{ isCourseSelected(course.id) ? '已选' : (getStudentCount(course) >= (course.maxStudents || 0) ? '名额已满' : '选择') }}
                  </span>
                </button>
              </div>
            </div>
...
export default {
  computed: {
    ...
    // 简化的已选课程处理
    processedSelectedCourses() {
      const studentId = String(this.userInfo?.id || '');
      if (!studentId || !Array.isArray(this.courses)) {
        return [];
      }
      
      console.log('计算已选课程，学生ID:', studentId);
      ...
      const selected = this.courses.filter(course => {
        ...
        if (typeof students === 'string') {
          try {
            ...
            console.warn('解析students失败:', e, course.students);
            students = [];
          }
        }
        
        const isSelected = Array.isArray(students) && students.map(String).includes(studentId);
        console.log(`课程${course.name} (${course.id}): students=${course.students}, isSelected=${isSelected}`);
        
        return isSelected;
      });
      
      console.log('筛选出的已选课程:', selected);
      return selected;
    },
    totalCredit() {
      if (!Array.isArray(this.processedSelectedCourses)) {
        return 0;
      }
      return this.processedSelectedCourses.reduce((sum, course) => {
  ...
  methods: {
    ...
    isCourseSelected(courseId) {
      ...
      return Array.isArray(students) && students.map(String).includes(studentId);
    },
    ...
    forceRefreshCourses() {
      ...
      this.loadCourses().then(() => {
        ...
        this.$nextTick(() => {
          ...
          console.log('已选课程数量:', this.processedSelectedCourses.length);
        });
      });
    },
  ...
  mounted() {
    console.log('StudentDashboardSimple已挂载');
